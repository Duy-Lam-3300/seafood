...
export default function Card({ product, ...rest }: Props) {
    return (
        <div {...rest} className="border border-gray-400 pb-4  shadow-md w-fit flex flex-col items-center relative">
            <img src={product.img} alt={product.enName} className=" h-72 w-72 object-cover " />
            <h2 className="text-lg font-bold mt-2">{product.enName} ({product.viName})</h2>
            ...
            <p className="font-bold absolute top-4 right-4 bg-red-500  h-11 w-11  text-white flex items-center justify-center">{product.discount}%</p>
        </div>
    );
}
